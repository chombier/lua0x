...
program. The file ~example/main.cpp~ is shown below:
...
~example/test.cpp~ for more details.
...
This will create =lib/libtest.so=. Then, run ~example/test.lua~ for a
use example.
